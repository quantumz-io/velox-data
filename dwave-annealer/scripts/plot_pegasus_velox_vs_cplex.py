...
def main() -> None:
    latex_plot(scale=2)
    df = pd.read_csv(RESULTS_DIR / "pegasus_velox_vs_cplex.csv")
    ...
    ax_left.set_ylabel(r"Pegasus graph index $m$", fontsize=fontsize_label)
    ax_left.set_yticks(x)
    ax_left.set_yticklabels([rf"${m}$" for m in plot_df["pegasus"].astype(int)], fontsize=fontsize_ticks)
    ...
    ax_right.legend(loc="lower right", frameon=False, fontsize=fontsize_text - 1)

    plt.tight_layout()
    out = Path(__file__).with_name("pegasus_velox_vs_cplex.pdf")
